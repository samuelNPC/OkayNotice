...
import { FileText, Wrench, ShoppingBag, LayoutGrid } from "lucide-react";
...
export default async function HomePage() {
  ...
  const latestPosts = latestSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as any[];

  // Fetch 6 Featured Articles
  const featuredQuery = query(postsRef, where("isFeatured", "==", true), limit(6));
  const featuredSnapshot = await getDocs(featuredQuery);
  const featuredPosts = featuredSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as any[];
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 mt-8">
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {/* The "More" button triggers the navbar hamburger menu using standard DOM interaction */}
            <button 
              onClick={() => {
