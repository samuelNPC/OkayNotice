        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 md:pt-10">
          ...
          <section className="hidden md:flex mb-14 text-left flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="grid grid-cols-3 gap-4 w-full lg:w-auto shrink-0">
              ...
              <Link href="/tools" className="flex items-center justify-center px-8 space-x-2 bg-white border border-slate-200 shadow-sm py-4 rounded-2xl hover:border-blue-300 hover:shadow-md transition-all">
                <Wrench size={20} className="text-pink-500" />
                <span className="font-bold text-slate-800">Tools</span>
              </Link>
              <Link href="/deals" className="flex items-center justify-center px-8 space-x-2 bg-white border border-slate-200 shadow-sm py-4 rounded-2xl hover:border-blue-300 hover:shadow-md transition-all">
                <ShoppingBag size={20} className="text-green-500" />
                <span className="font-bold text-slate-800">Deals</span>
              </Link>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
              {displayFeatured.map((post) => (
                <Link 
                  key={post.id} 
                  href={`/blog/${post.slug}`}
                  className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 flex flex-col hover:border-blue-300 transition-colors"
                >
          ...
          <section className="mb-16">
            ...
            <div className="hidden md:block">
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {latestPosts.map((post) => (
                  <Link 
                    key={post.id} 
                    href={`/blog/${post.slug}`} 
                    className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all"
                  >
              ...
              <Link 
                href="/blog" 
                className="flex items-center justify-center w-full py-4 bg-white border border-blue-200 text-blue-700 font-bold rounded-2xl hover:bg-blue-50 transition-colors shadow-sm"
              >
                View All Blogs &rarr;
              </Link>
            </div>
